...
import { situacaoShopee } from "./shopee";
...
export function situacao(env) {
  if (env.canal === "shopee") return situacaoShopee(env);
  ...
  if (st === "pending" || st === "handling" || st === "to_be_agreed") {
    return {
      etapa: "futuras",
      rotulo: "Ainda vem por aí",
      ordem: 4,
      acao: "O Mercado Livre ainda não liberou. Nada a fazer agora.",
    };
  }
...
export async function listarExpedicao(dias = 15, canal = null) {
  const desde = new Date(Date.now() - dias * 86400000);
  const r = await query(
    `SELECT e.*, p.data_criacao AS pedido_em, p.comprador, p.total_pedido, p.canal
       FROM envios e
  ...
  return r.rows.map((e) => {
    ...
    return {
      ...
      nf_erro: e.nf_erro,
      vezes_impresso: Number(e.vezes_impresso || 0),
      link_ml:
        e.canal === "shopee"
          ? `https://seller.shopee.com.br/portal/sale/order/${e.order_id}`
          : `https://www.mercadolivre.com.br/vendas/${e.order_id}/detalhe`,
      itens,
...
/**
 * Marca ou desmarca envios. Desfazer existe porque errar é normal:
 * imprimiu sem querer, marcou o pacote errado como enviado, a etiqueta
 * saiu borrada. Nada aqui é irreversível.
 */
export async function marcar(shipmentIds, campo, desfazer = false) {
  const colunas = {
    impresso: "impresso_em",
    despachado: "despachado_em",
  };
  const coluna = colunas[campo] || "impresso_em";
  const valor = desfazer ? "NULL" : "now()";

  // Desfazer o envio também limpa nada além dele; desfazer a impressão
  // mantém o despacho, porque são decisões independentes.
  await query(
    `UPDATE envios SET ${coluna} = ${valor} WHERE shipment_id = ANY($1::text[])`,
    [shipmentIds.map(String)],
  );
}

/** Conta quantas vezes cada etiqueta já foi impressa, para avisar na tela. */
export async function registrarImpressao(shipmentIds) {
  await query(
    `UPDATE envios
        SET impresso_em = now(),
            vezes_impresso = COALESCE(vezes_impresso, 0) + 1
...
/**
 * Lista de separação: agrupa por SKU o que precisa sair da prateleira.
 * Numa expedição real é isso que se leva para o estoque, não a etiqueta.
 */
export async function listaSeparacao(shipmentIds) {
  const r = await query(
    `SELECT pi.item_id, pi.variation_id, pi.titulo, pi.sku,
            SUM(pi.quantidade)::int AS unidades,
            COUNT(DISTINCT e.shipment_id)::int AS pacotes
       FROM envios e
       JOIN pedido_itens pi ON pi.order_id = e.order_id
      WHERE e.shipment_id = ANY($1::text[])
      GROUP BY pi.item_id, pi.variation_id, pi.titulo, pi.sku
  ...
  const linhas = r.rows.map((x) => ({
    ...x,
    // Onde o produto fica guardado, se o vendedor tiver cadastrado.
    localizacao: null,
  }));

  return {
    linhas,
    total_unidades: linhas.reduce((t, l) => t + l.unidades, 0),
    total_skus: linhas.length,
    total_pacotes: shipmentIds.length,
  };
}
