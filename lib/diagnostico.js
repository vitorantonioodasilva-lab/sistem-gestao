...
export function montarDossie(painel) {
  ...
  return {
    ...
    resultado: {
      ...
      tarifa_do_marketplace: d.tarifa,
    ...
    // Cada canal com o seu resultado. `diferenca_repasse` compara a nossa
    // conta com o que o marketplace diz que repassou: quando abre, a
    // modelagem de tarifa daquele canal está incompleta e a análise precisa
    // dizer isso em vez de tratar o lucro como certo.
    canais: (painel.canais || []).map((c) => ({
      canal: c.canal === "shopee" ? "Shopee" : "Mercado Livre",
      pedidos: c.pedidos,
      receita: c.receita,
      tarifa: c.tarifa,
      frete: c.frete,
      custo_dos_produtos: c.cmv,
      publicidade: c.ads,
      lucro: c.lucro,
      margem_pct: Number(c.margem_pct?.toFixed(1)),
      participacao_na_receita_pct: Number(c.pct_receita?.toFixed(1)),
      ticket_medio: c.ticket_medio,
      repasse_informado_pelo_marketplace: c.repasse_informado,
      diferenca_para_a_nossa_conta:
        c.diferenca_repasse === null ? null : Number(c.diferenca_repasse.toFixed(2)),
    })),
    publicidade: {
...
const INSTRUCAO = `Você é um analista de e-commerce que assessora um vendedor brasileiro que vende no Mercado Livre e na Shopee.
...
- Compare os canais quando fizer diferença: margem, ticket e participação em "canais". Diga em qual canal cada produto rende mais.
- A publicidade sincronizada é só a do Mercado Livre. Não atribua gasto de anúncio à Shopee.
- Se "diferenca_para_a_nossa_conta" de um canal não for zero nem nulo, avise: o lucro daquele canal está incerto porque a tarifa modelada não bate com o repasse informado pelo marketplace.
