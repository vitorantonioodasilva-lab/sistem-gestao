...
async function examinar(arquivo, comDocumentos = false) {
  ...
  doc.getPages().forEach((pagina, indice) => {
    ...
    const { width: lp, height: ap } = pagina.getSize();
    const folhaInteira = { x0: 0, y0: 0, x1: lp, y1: ap };
    ...
    // Declaração de conteúdo e lista de separação são papel A4. Por padrão
    // saem à parte, mas a declaração precisa ir colada no pacote junto com a
    // etiqueta — então dá para mandar para a térmica também. Aqui vale a
    // página inteira: procurar contorno de etiqueta num documento acharia
    // alguma tabela e recortaria errado.
    ...
    for (const caixa of achadas.length ? achadas : [folhaInteira]) {
      recortes.push({ pagina, caixa, giro, identidade, tamanho: medir(caixa) });
    }
  });
...
export async function converter(arquivos, opcoes = {}) {
  ...
  const incluirDocumentos = cfg.documentos === "incluir";
  ...
  for (const arquivo of arquivos) {
    ...
    let lido;
    try {
      lido = await examinar(arquivo, incluirDocumentos);
    } catch {
    ...
    if (lido.documentos.length) {
      ...
      item.aviso = incluirDocumentos
        ? `${lido.documentos.length} página(s) de ${quais} entraram encolhidas na etiqueta — confira se dá para ler.`
        : `${lido.documentos.length} página(s) de ${quais} ficaram de fora — são A4 e você baixa à parte.`;
    }
    ...
    for (const grupo of porTamanho(lido.recortes)) {
      // Documento A4 vai sempre sozinho na folha — já sai reduzido, dividir
      // em grade tornaria a leitura impossível.
      const ehDocumento = grupo.itens[0].documento;
      const modo = ehDocumento
        ? "unica"
        : cfg.modo === "auto"
          ? modoAutomatico(grupo.tamanho, cfg.midia)
          : cfg.modo;
