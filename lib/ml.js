...
async function upsertProduto({ item_id, variation_id, sku, titulo, estoque, preco, listing_type }) {
  await query(
    `INSERT INTO produtos (canal, item_id, variation_id, sku, titulo, estoque_atual, preco_anuncio, listing_type, atualizado_em)
     VALUES ('ml',$1,$2,$3,$4,$5,$6,$7, now())
...
async function gravarPedido(order, token) {
  ...
  for (const it of order.order_items || []) {
    ...
    await query(
      `INSERT INTO produtos (canal, item_id, variation_id, sku, titulo, preco_anuncio, listing_type)
       VALUES ('ml',$1,$2,$3,$4,$5,$6)
       ON CONFLICT (canal, item_id, variation_id) DO NOTHING`,
      [
        it.item?.id,
        variation,
        it.item?.seller_sku || it.item?.seller_custom_field || null,
