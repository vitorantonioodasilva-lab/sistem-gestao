...
-- A tabela envios nasceu com as chaves em TEXT enquanto pedidos usa BIGINT,
-- o que fazia o Postgres recusar o JOIN com "operator does not exist: text = bigint".
-- Converte quem ja tinha a tabela antiga. Idempotente.
DO $migra$
DECLARE
  col TEXT;
BEGIN
  FOREACH col IN ARRAY ARRAY['shipment_id','order_id','pack_id'] LOOP
    IF EXISTS (
      ...
      EXECUTE format(
        'ALTER TABLE envios ALTER COLUMN %I TYPE BIGINT USING NULLIF(btrim(%I), '''')::bigint',
        col, col
      );
    END IF;
  END LOOP;
