...
export default function Pagina() {
  return (
    <Suspense fallback={<p style={{ fontFamily: 'var(--mono)' }}>Carregando…</p>}>
      <Config />
    </Suspense>
  );
}
...
function Config() {
  ...
  const faltaAmbiente = !amb.client_id || !amb.client_secret || !amb.redirect_uri;
  const totalFixo = (d.custos_fixos || []).reduce((a, c) => a + Number(c.valor_mensal), 0);
    ...
    <>
      {params.get('conectado') && (
        <div className="aviso">
          ...
          Agora volte ao painel e clique em “Atualizar do ML” para trazer o histórico.
        </div>
      )}
      {params.get('erro') && <div className="alerta">Falha na conexão: {params.get('erro')}</div>}
      ...
      <section className="secao">
        ...
        {faltaAmbiente ? (
          <div className="aviso">
            ...
            <ul style={{ fontFamily: 'var(--mono)', fontSize: 12.5, lineHeight: 1.9 }}>
            ...
            O valor de <code>ML_REDIRECT_URI</code> precisa ser idêntico ao cadastrado na sua
            aplicação no DevCenter, incluindo <code>https://</code> e sem barra final.
          </div>
        ) : d.conta ? (
          <div className="fita" style={{ maxWidth: 460 }}>
            ...
            <div className="fita-linha">
              ...
              <span>{new Date(d.conta.conectado_em).toLocaleDateString('pt-BR')}</span>
            </div>
            <div className="fita-linha">
            ...
            <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
              <a className="botao" href="/api/ml/auth">Reautorizar</a>
              <a className="botao" href="/api/ml/sync">Sincronizar agora</a>
            </div>
          </div>
        ) : (
          <div>
            <p style={{ maxWidth: 620, lineHeight: 1.6 }}>
              Você será levado ao Mercado Livre para autorizar o acesso de leitura aos seus
              pedidos, anúncios e envios. Entre com a conta principal de vendedor — login de
              colaborador não gera autorização válida.
            </p>
            <a className="botao" href="/api/ml/auth" style={{ background: 'var(--marcador)' }}>
        ...
        <div className="form-linha">
          <div className="campo">
            ...
            <input
              ...
              onBlur={(e) => salvar({ config: { aliquota_imposto: e.target.value } })}
              className="mini"
            />
          </div>
          <div className="campo">
            ...
            <input
              ...
              onBlur={(e) => salvar({ config: { provisao_devolucao: e.target.value } })}
              className="mini"
            />
          </div>
          <div className="campo" style={{ minWidth: 260 }}>
            ...
            <select
              ...
              onChange={(e) => salvar({ config: { sale_fee_por_unidade: e.target.value } })}
            >
              <option value="true">é por unidade (multiplicar pela quantidade)</option>
          ...
          <div className="campo">
            ...
            <input
              ...
              onBlur={(e) => salvar({ config: { dias_historico: e.target.value } })}
              className="mini"
            />
          </div>
        </div>
        <p style={{ fontSize: 13, color: 'var(--tinta-fraca)', maxWidth: 680, lineHeight: 1.6 }}>
          Compare o total de tarifa do painel com a fatura do mês no Mercado Livre. Se der o dobro
          ou a metade, é a opção do <code>sale_fee</code> acima que precisa mudar.
        </p>
      </section>
      ...
      <section className="secao">
        <div className="secao-titulo">
          ...
          <span style={{ textTransform: 'none', letterSpacing: 0 }}>total {brl(totalFixo)}/mês</span>
        </div>
              ...
              {!d.custos_fixos?.length && (
                <tr>
                  <td colSpan={3} style={{ color: 'var(--tinta-fraca)' }}>
                    Sem custos fixos. Aluguel, salários, pró-labore, sistemas e contador entram aqui.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <tbody>
            {(d.log || []).map((l) => (
              <tr key={l.id}>
                ...
                <td style={{ color: 'var(--prejuizo)', fontSize: 12 }}>{l.erro || ''}</td>
              </tr>
            ))}
            {!d.log?.length && (
              <tr>
                <td colSpan={4} style={{ color: 'var(--tinta-fraca)' }}>Nada sincronizado ainda.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <p className="rodape">
        ...
        <code>{amb.redirect_uri ? amb.redirect_uri.replace('/api/ml/callback', '/api/ml/webhook') : 'https://seu-app.vercel.app/api/ml/webhook'}</code>
        <br />
        Painel {amb.senha_painel ? 'protegido por senha.' : 'SEM senha — defina APP_PASSWORD na Vercel.'}
      </p>
    </>
  );
}
