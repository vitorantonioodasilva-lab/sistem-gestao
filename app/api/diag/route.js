...
export async function GET() {
  ...
  if (!url) {
    out.banco.erro = 'DATABASE_URL não existe neste deploy. Cadastre na Vercel e faça redeploy.';
  ...
  try {
    ...
    out.banco.erro = 'DATABASE_URL não é uma URL válida. Confira se não colou com aspas ou espaço.';
    return NextResponse.json(out, { status: 200 });
  }
...
function dica(e) {
  ...
  if (e.code === '3D000') return 'O banco citado na URL não existe nesse servidor.';
  if (e.code === '42501') return 'Usuário sem permissão para criar tabelas no schema public.';
